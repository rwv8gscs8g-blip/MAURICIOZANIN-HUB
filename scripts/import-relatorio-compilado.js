...
const parsePerguntasChave = (sectionText) => {
  ...
  if (answers.tramitacaoEletronicaNota === null) {
    answers.tramitacaoEletronicaNota = 0;
  }
...
async function main() {
  ...
  if (!filePath) {
    console.error("Uso: node scripts/import-relatorio-compilado.js <caminho_txt>");
    process.exit(1);
  }
  ...
  const markerRegex = /2\.\d+\s+MUNICIPIO DE\s+([^\n]+)/gi;
  const markers = [];
  let match;
  ...
  if (markers.length === 0) {
    console.error("Nenhum município encontrado.");
    process.exit(1);
  }
  ...
  for (let i = 0; i < markers.length; i++) {
    ...
    const section = text.slice(current.index, next ? next.index : text.length);
    const municipioNome = cleanText(current.name.replace(/\s+\d+$/, ""));
    const municipioKey = normalizeText(municipioNome);

    const hasFormulario = section.includes("FORMULÁRIO DE DIAGNÓSTICO") ||
      section.includes("Formulário Estruturado");
    if (!hasFormulario) {
      continue;
    }

    if (processed.has(municipioKey)) {
      continue;
    }
    processed.add(municipioKey);
    ...
    const respondentName = respondents.map((r) => r.name).join(" | ");
    ...
    for (let idx = 0; idx < EIXOS.length; idx++) {
      ...
      const parsed = parseEixo(section, eixo.title, nextTitles);
      if (!parsed) continue;
      eixosData.push({
        eixoKey: eixo.key,
        ...parsed.resposta,
    ...
    const perguntas = parsePerguntasChave(section);
    ...
    const diagnostico = await prisma.diagnostico.create({
      data: {
        ...
        cnpj: "",
        respondentName: respondentName || municipioNome,
        respondentEmail,
        respondentPhone,
        status,
        consentLgpd: true,
